...
import { Archive, ArrowDown, ArrowUp, Eye, EyeOff, GripVertical, Pencil, RefreshCw, Rocket, Save, Star, Trash2 } from "lucide-react";
...
export function ProjectManagement() {
  ...
  function moveProjectToIndex(fromIndex: number, toIndex: number) {
    const item = items[fromIndex];
    if (!item || toIndex < 0 || toIndex >= items.length || fromIndex === toIndex) {
      return;
    }
    const direction = toIndex > fromIndex ? 1 : -1;
    void patchProject(item.id, { order: item.order + direction }, `Proyecto reordenado: ${item.name}.`);
  }

  function handleProjectDrop(toIndex: number) {
    if (draggedProjectIndex === null) {
      return;
    }
    moveProjectToIndex(draggedProjectIndex, toIndex);
    ...
    <div className="grid gap-6">
      ...
      <section className="overflow-x-auto rounded-lg border border-border">
        <div className="min-w-[980px]">
          <div className="grid grid-cols-[40px_1.2fr_1fr_130px_140px_300px] border-b border-border bg-muted/40 p-3 text-sm font-medium">
          ...
          {items.length ? items.map((item, index) => (
            <div
              key={item.id}
              className="grid grid-cols-[40px_1.2fr_1fr_130px_140px_300px] gap-3 border-b border-border p-3 text-sm last:border-b-0"
              data-cms-project-id={item.id}
