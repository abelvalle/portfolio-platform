...
Estado actualizado: 2026-06-11 19:13 CEST.
...
### Suite raiz tras bloque mensajes/dashboard

- Ejecutada la suite raiz despues de export CSV de mensajes, cohorts comparativos y accion masiva de mensajes.
- Ejecutado build raiz de produccion para API y web.
- Resultado: `24` suites y `119` tests API, smoke web y build Next/Nest correctos.
...
### Revalidacion audit Next/PostCSS

- Revalidado `npm audit --audit-level=moderate`: siguen 2 vulnerabilidades moderadas por `postcss <8.5.10` dentro de `next`.
- `npm.cmd view next@latest version dependencies.postcss` confirma `next@latest = 16.2.7` y `dependencies.postcss = 8.4.31`.
- `npm.cmd ls next postcss --workspace apps/web` confirma que Tailwind/shadcn ya usan `postcss@8.5.15`, pero Next mantiene su dependencia interna vulnerable.
- No se aplica `npm audit fix --force` porque sigue proponiendo downgrade rompedor a `next@9.3.3`.
...
### Estado LinkedIn conectado en settings

- `/admin/settings` consulta `GET /integrations/linkedin/status` desde el cliente web.
- La tarjeta LinkedIn muestra OAuth configurado/pendiente, cuenta sincronizada/sin cuenta, share on/off, scopes y ultima sincronizacion.
- El estado visible evita mostrar email o tokens y mantiene la tabla de endpoints de integracion.
...
### Suite raiz tras bloque LinkedIn

- Ejecutada la suite raiz despues de persistencia segura LinkedIn y estado conectado en settings.
- Ejecutado build raiz de produccion para API y web.
- Resultado: `24` suites y `119` tests API, smoke web y build Next/Nest correctos.
...
### Labels localizados en exportacion CV

- El exportador HTML/PDF/DOCX usa `CvVersion.language` para elegir labels ES/EN de secciones.
- Las etiquetas de resumen, experiencia, formacion/certificaciones, idiomas, proyectos y certificado se localizan sin traducir contenido profesional.
- Las generaciones publicas, ATS y de versiones CV pasan `language` al renderer.

Verificacion realizada en este hito:

- `npm.cmd --prefix apps/api run test -- cv-export.service.spec.ts cv.service.spec.ts cv-version.service.spec.ts`
- `npm.cmd --prefix apps/api run lint`
- `npm.cmd run build:api`

### Reintentos automaticos diferidos de webhooks

- `ContactWebhookService` agenda reintentos diferidos para entregas fallidas de `contact.message.created`.
- La politica se controla con `CONTACT_WEBHOOK_RETRY_ATTEMPTS` y `CONTACT_WEBHOOK_RETRY_DELAY_MS`, documentadas en `.env.example`.
- Cada intento mantiene auditoria sin URL, secreto ni payload, y registra `retryAttempt` cuando aplica.
- `/admin/settings` muestra reintentos configurados y el intento en el historial de entregas.
...
### Suite raiz tras bloque AppModule

- Ejecutada la suite raiz despues de navegacion sensible a modulos, permisos endurecidos y saneamiento de escrituras `AppModule`.
- Ejecutado build raiz de produccion para API y web.
- Resultado: `26` suites y `125` tests API, smoke web y build Next/Nest correctos.
...
### Cobertura e2e drag/drop CMS

- El e2e admin ejecuta drag/drop real sobre filas de proyectos usando los handlers del CRUD.
- La cobertura ya no se limita a comprobar `draggable=true`; valida que el drop dispara el reordenado.
- La deuda de drag/drop CMS se actualiza para reflejar que las entidades principales ya tienen filas arrastrables y fallback por botones.
...
### Guard explicito de storage externo

- `MediaStorageService.createReadStream` bloquea descargas cuando `MEDIA_STORAGE_PROVIDER` no es `local`.
- Evita tratar claves remotas como rutas locales mientras no exista adaptador S3/R2/Supabase Storage.
- Añadido test unitario para provider externo no implementado.
...
### Embudo Analytics configurable por API

- `GET /analytics/funnel` acepta `steps` como CSV de 2 a 6 tipos de evento.
- El endpoint conserva el embudo por defecto cuando no se envia `steps`.
- Añadidos tests para embudo custom, ratios y validacion de pasos invalidos.
- `docs/api.md` documenta el nuevo parametro.
...
### Presets UI para embudo Analytics

- `/admin/analytics` permite elegir presets de embudo desde un selector.
- El cliente web envia `steps` a `GET /analytics/funnel` y actualiza titulo, badge y cards del embudo.
- El e2e valida el preset `Landing -> Proyecto -> Contacto` con datos mockeados desde API.
...
### Revalidacion audit Next/PostCSS 2026-06-07
...
### Suite raiz tras bloque Analytics

- Ejecutada la suite raiz despues de embudo configurable por API, presets UI y revalidacion audit Next/PostCSS.
- Ejecutado build raiz de produccion para API y web.
- Resultado: `26` suites y `128` tests API, smoke web y build Next/Nest correctos.
...
### Contrato mock de adaptador IA CV

- `CvAiAdapterService` queda cubierto con fetch mockeado cuando `CV_AI_ADAPTER_URL` esta configurado.
- El test valida que se envia fuente saneada sin email/enlaces/notas privadas y con `Authorization` cuando hay API key.
- Las sugerencias externas se normalizan, se marcan `pendingReview` y se ignoran campos no reconocidos.
- Tambien se valida fallback `null` cuando el proveedor externo responde error.

Verificacion realizada en este hito:

- `npm.cmd --prefix apps/api run test -- cv-ai-adapter.service.spec.ts`
- `npm.cmd --prefix apps/api run lint`
- `npm.cmd run build:api`
...
### Copy publico conectado a i18n persistente

- `portfolioClient.publicCopy(locale)` carga `GET /translations/public` y aplica entradas sobre `publicCopy` local.
- La mezcla solo reemplaza rutas string ya existentes, evitando crear estructura inesperada desde traducciones mal formadas.
- Landing, contacto y CV online cargan snapshot y copy persistido en paralelo.
- Si la API no responde, las rutas publicas mantienen el fallback local existente.
...
### Suite raiz tras bloque Analytics, CV e i18n

- Ejecutada la suite raiz despues de embudos persistentes, trazabilidad saneada de adaptaciones CV, CMS i18n y copy publico conectado.
- Ejecutado build raiz de produccion para API y web.
- Resultado: `27` suites y `138` tests, smoke web y build Next/Nest correctos.
...
### Diccionario i18n anidado por API

- `TranslationsService.dictionary` convierte entradas `locale/namespace/key` en un objeto anidado consumible por frontend.
- La API expone `GET /translations/public/dictionary` y `GET /translations/dictionary`.
- El modo publico fuerza `visible=true` y retira el prefijo `public.` para mantener estable el contrato de copy.
- Tests unitarios y e2e cubren modo publico forzado y estructura anidada.
...
### Contrato HTTP de settings webhook

- Añadido e2e de `ContactMessagesController` para `GET|PATCH /contact-messages/webhook/settings`.
- El test valida que settings no exponen secreto, que `PATCH` elimina campos desconocidos como `secret` y que URL invalida devuelve `400`.
...
### Suite raiz tras cola webhook

- Ejecutada la suite raiz despues de la cola persistente de reintentos webhook.
- Ejecutado build raiz de produccion para API y web.
- Resultado: `27` suites y `141` tests, smoke web y build Next/Nest correctos.
...
### Seed i18n publico ampliado

- El seed `TranslationEntry` cubre ahora microcopy publico ES/EN para intro, hero, command palette, about, experiencia, proyectos, etiquetas de educacion, contacto y CV online.
- Las nuevas entradas salen del fallback `publicCopy` existente, sin inventar contenido profesional.
- Los labels de `sections` se cierran en el hito siguiente al requerir soporte especifico para arrays indexados.
...
### Sections i18n desde diccionario

- `applyPublicDictionary` soporta arrays indexados de forma conservadora para claves como `sections.0.label`.
- El seed publica labels ES/EN de las seis secciones publicas sin tocar `id` ni `number`.
- Indices inexistentes o estructuras no compatibles se ignoran para evitar que traducciones mal formadas creen UI nueva.
...
### Estado visible del worker webhook

- `ContactWebhookService.status/settings` exponen `retryWorkerEnabled` y `retryWorkerIntervalMs`.
- `/admin/settings` muestra si el worker de reintentos esta activo y su intervalo efectivo.
- La API conserva secretos fuera de respuesta y solo expone configuracion operativa no sensible.
...
### Suite raiz tras worker webhook visible

- Ejecutada la suite raiz despues de exponer estado del worker y coordinar fallback local.
- Ejecutado build raiz de produccion para API y web.
- Resultado: `28` suites y `146` tests, smoke web y build Next/Nest correctos.
...
### Suite raiz tras contrato worker webhook

- Ejecutada la suite raiz despues de cubrir el contrato HTTP del estado del worker.
- Ejecutado build raiz de produccion para API y web.
- Resultado: `28` suites y `146` tests, smoke web y build Next/Nest correctos.
...
### Cron externo para reintentos webhook

- Añadido `POST /contact-messages/webhook/retries/cron` para procesar cola desde cron externo.
- El endpoint usa `X-Portfolio-Cron-Secret` y `CONTACT_WEBHOOK_RETRY_CRON_SECRET`, sin requerir sesion admin.
- La comparación del secreto se realiza sobre hashes SHA-256 con `timingSafeEqual`.
- `.env.example`, `docs/api.md` y `docs/deployment.md` documentan el contrato.
...
### Auditoria de cron webhook

- Las ejecuciones validas del cron externo registran `AuditLog` con accion `contact.webhook.retry_cron`.
- La auditoria solo persiste `processed` y no guarda header, secreto ni payload.
- Fallos de auditoria no bloquean el procesamiento de la cola.
...
- Añadido `AnalyticsRetentionWorker` dentro de `AnalyticsModule`.
- El worker solo arranca si `ANALYTICS_RETENTION_DAYS` es positivo y `ANALYTICS_RETENTION_WORKER_ENABLED` no es `false`.
- La purga periodica reutiliza `AnalyticsService.pruneRetention` y respeta `ANALYTICS_RETENTION_WORKER_INTERVAL_MS`.
- `.env.example`, `docs/api.md` y `docs/deployment.md` documentan las variables.

Verificacion realizada en este hito:

- `npm.cmd --prefix apps/api run test -- analytics-retention.worker.spec.ts`
- `npm.cmd --prefix apps/api run test -- analytics.service.spec.ts`
- `npm.cmd --prefix apps/api run lint`
- `npm.cmd run build:api`
...
### Deployment del worker Analytics

- `docs/deployment.md` documenta el worker de retencion analytics en single instance y multi-replica.
- Se recomienda una sola replica dedicada o desactivar el worker y delegar en tarea programada externa.
...
### Objetivos KPI persistentes Analytics

- Añadido modelo Prisma `AnalyticsGoal` con migracion `000028_add_analytics_goals`.
- Añadidos endpoints protegidos `GET|POST|PATCH|DELETE /analytics/goals`.
- Añadido `GET /analytics/goals/progress` para calcular `count`, `progressRate` y `achieved` por rango de fechas.
- El seed crea objetivos `sample/demo` editables sin convertirlos en KPIs profesionales reales.
- `/admin/analytics` muestra objetivos KPI, progreso, creacion, edicion, ocultacion y labels accesibles para botones duplicados.
- `docs/api.md` documenta el contrato REST y ejemplo de respuesta.
...
### Validacion guiada de secreto webhook

- Añadido `POST /contact-messages/webhook/secret/validate` protegido con `manage_messages`.
- La comparacion usa hashes SHA-256 y `timingSafeEqual`; no devuelve ni persiste el candidato ni el secreto configurado.
- `/admin/settings` incorpora un campo temporal para validar `CONTACT_WEBHOOK_SECRET`, limpia el valor tras validar y muestra header/algoritmo.
- `docs/api.md` documenta el endpoint y el contrato no sensible.
...
### Revalidacion Next/PostCSS

- Revalidado `next@latest` desde npm: sigue en `16.2.7`.
- Revalidada dependencia interna `next@latest dependencies.postcss`: sigue en `8.4.31`.
- `npm audit --audit-level=moderate` mantiene 2 vulnerabilidades moderadas en la cadena `next`/`postcss`.
- No se aplica `npm audit fix --force` porque todavia propone downgrade rompedor a `next@9.3.3`.
...
### Suite raiz tras objetivos KPI en Dashboard admin

- Ejecutada la suite raiz despues de exponer objetivos KPI en el dashboard admin.
- Ejecutado build raiz de produccion para API y web.
- Resultado: `29` suites y `158` tests, smoke web y build Next/Nest correctos.
- Observacion: el build web sigue mostrando `DEP0205 module.register()` desde la cadena de Next/Node, sin bloquear compilacion.
...
### Idiomas granulares en CV Manager

- Versiones CV incorpora formulario granular para `languages`.
- El bloque permite seleccionar idioma existente o nuevo, editar nombre y nivel, duplicar y borrar items.
- La aplicacion preserva objetos existentes y mantiene el bloque de texto como edicion rapida compatible.
- El e2e admin cubre aplicar, duplicar, eliminar y crear idiomas granulares.
...
### Prueba manual de email contacto

- Anadido `POST /contact-messages/email/test` protegido por `manage_messages`.
- La prueba usa un mensaje sintetico sin datos personales de contactos y reutiliza el proveedor email configurado.
- `/admin/settings` incorpora boton "Probar email" y mantiene visible el resultado de la accion tras refrescar estado.
- `docs/api.md` y `docs/deployment.md` documentan la prueba operativa post-deploy.

Verificacion realizada en este hito:

- `npm.cmd --prefix apps/api run test -- contact-email-notification.service.spec.ts`
- `npm.cmd --prefix apps/api run test:e2e -- contact-webhook.e2e-spec.ts`
- `npm.cmd --prefix apps/web run test:e2e -- landing.spec.ts --project=chromium -g "admin"`
- `npm.cmd --prefix apps/api run lint`
- `npm.cmd --prefix apps/web run lint`
...
### Suite raiz tras prueba email

- Ejecutada la suite raiz despues de anadir la prueba sintetica de proveedor email.
- Ejecutado build raiz de produccion para API y web.
- Resultado: `30` suites y `163` tests, smoke web y build Next/Nest correctos.
- Observacion: el build web sigue mostrando `DEP0205 module.register()` desde la cadena de Next/Node, sin bloquear compilacion.
...
### Scanner externo opcional para Media

- `MediaStorageService` puede llamar un scanner HTTP externo antes de escribir uploads locales cuando `MEDIA_EXTERNAL_SCAN_URL` esta configurado.
- El payload enviado al scanner incluye nombre, MIME, tamano, SHA-256 y contenido base64; no escribe el archivo si el scanner falla o devuelve un veredicto no limpio.
- `GET /media/storage/status` expone `externalScanEnabled` y `externalScanConfigured`.
- `/admin/media` muestra badge de antivirus externo y el e2e admin lo cubre.
- `.env.example`, README, `docs/api.md` y `docs/deployment.md` documentan `MEDIA_EXTERNAL_SCAN_*`.
...
### Suite raiz tras scanner media

- Ejecutada la suite raiz despues de anadir scanner HTTP externo opcional para media.
- Ejecutado build raiz de produccion para API y web.
- Resultado: `30` suites y `165` tests, smoke web y build Next/Nest correctos.
- Observacion: el build web sigue mostrando `DEP0205 module.register()` desde la cadena de Next/Node, sin bloquear compilacion.
...
### Suite raiz tras prueba scanner media

- Ejecutada la suite raiz despues de anadir la prueba sintetica del scanner externo de media.
- Ejecutado build raiz de produccion para API y web.
- Resultado: `30` suites y `167` tests, smoke web y build Next/Nest correctos.
- Observacion: el build web sigue mostrando `DEP0205 module.register()` desde la cadena de Next/Node, sin bloquear compilacion.
...
### Suite raiz tras health checks API

- Ejecutada la suite raiz despues de anadir liveness/readiness para despliegue API.
- Ejecutado build raiz de produccion para API y web.
- Resultado: `30` suites y `170` tests, smoke web y build Next/Nest correctos.
- Observacion: el build web sigue mostrando `DEP0205 module.register()` desde la cadena de Next/Node, sin bloquear compilacion.
...
### Suite raiz tras Swagger configurable

- Ejecutada la suite raiz despues de condicionar Swagger por entorno.
- Ejecutado build raiz de produccion para API y web.
- Resultado: `32` suites y `174` tests, smoke web y build Next/Nest correctos.
- Observacion: el build web sigue mostrando `DEP0205 module.register()` desde la cadena de Next/Node, sin bloquear compilacion.
...
### Suite raiz tras guard de secretos

- Ejecutada la suite raiz despues de anadir validacion de secretos placeholder en produccion.
- Ejecutado build raiz de produccion para API y web.
- Resultado: `33` suites y `177` tests, smoke web y build Next/Nest correctos.
- Observacion: el build web sigue mostrando `DEP0205 module.register()` desde la cadena de Next/Node, sin bloquear compilacion.
...
### CI remoto verde tras filtro documentacion

- Revalidado GitHub Actions despues de anadir el filtro `paths-ignore`.
- Run `27085389773` completado correctamente en `develop`.
- Jobs verdes: `Quality gates` y `Web e2e smoke`.
...
### CI remoto verde tras Dependabot

- Revalidado GitHub Actions despues de anadir `.github/dependabot.yml`.
- Run `27085502994` completado correctamente en `develop`.
- Jobs verdes: `Quality gates` y `Web e2e smoke`.
...
### Politica verificable de npm audit

- Anadido `scripts/audit-policy.mjs` y script raiz `npm run audit:policy`.
- La politica permite solo el hallazgo moderado conocido de `postcss` propagado por `next`.
- Cualquier vulnerabilidad nueva, severidad distinta o advisory no reconocido falla con salida no cero.
- README incorpora el comando en la rutina de calidad local.
...
- `npm.cmd run build:api`
- `git diff --check`
- `gh run view 27364263769 --repo abelvalle/portfolio-platform --json status,conclusion,url,jobs`
- `gh run view 27364263827 --repo abelvalle/portfolio-platform --json status,conclusion,url,jobs`
- Resultado remoto: `success`
...
- Mensajes UI avanzada: la bandeja está conectada con filtros API por fecha/estado, vista detalle, confirmación de borrado, respuesta `mailto`, export CSV server-side, accion masiva filtrada de leido/no leido, privacidad configurable de metadata técnica y notificacion email HTTP opcional con estado y prueba sintetica visibles en admin; falta validarla con credenciales de un proveedor email real.
...
- Versiones CV UI avanzada: el JSON estructurado ya se puede editar con validación semántica mínima, confirmación para cambios grandes, preservacion de campos ricos al aplicar listas simples, bloques de resumen/skills/idiomas/proyectos/educación/certificaciones/experiencia/secciones, formularios granulares de experiencia/skills/proyectos/educacion/certificaciones con alta, duplicado y borrado de items, orden exportable `sectionOrder` con drag/drop visual y fallback por botones, duplicado y reordenado drag/drop de secciones personalizadas, borrador/revision/publicacion para `structuredJson` y draft/publish de metadatos no JSON; las entidades CMS principales ya tienen filas drag/drop con fallback por botones.
- Editor CV por bloques: el editor principal está conectado a campos básicos y Versiones CV ya tiene bloques de resumen/skills/idiomas/proyectos/educación/certificaciones/experiencia/secciones, duplicado de versiones, orden de bloques exportable, alta/duplicado/borrado granular multi-item, edición granular de experiencia, skills, idiomas, proyectos, educacion y certificaciones, listas internas visuales para responsabilidades/logros de experiencia, tecnologias internas de proyectos, URL externa de educacion e ID de credencial de certificaciones; los metadatos avanzados de formacion ya se exportan en HTML/PDF y DOCX. Falta granularidad equivalente solo si se incorporan bloques internos mas ricos.
...
- NPM audit: quedan 2 vulnerabilidades moderadas en la cadena `next`/`postcss` confirmadas con `npm.cmd audit --audit-level=moderate`; revalidado el 2026-06-07 06:41 CEST que `next@latest` sigue siendo `16.2.7` y todavia depende de `postcss@8.4.31`, mientras Tailwind/shadcn ya usan `postcss@8.5.15`; no se aplica `audit fix --force` porque propone un downgrade rompedor a `next@9.3.3`. `npm run audit:policy` permite solo esta deuda conocida y falla ante hallazgos nuevos.

## Próximos hitos priorizados

1. Preparar el siguiente bloque funcional no dependiente de Docker/Postgres real.
